...
SIM_DB_PATH = '/project/cobey/ccmproject-storage/2015-12-08-corr-ident-sweep-sims/results_gathered.sqlite'
...
def get_ccm_settings():
    ...
    timeseries_y_label= 'annual cases' # below.

    years = 1000                        # Number of years to use at end of time series
    ...
    max_theiler_window = 10             # Maximum window for excluding nearest neighbors temporally

    max_prediction_horizon = 20         # Twice the Theiler window is used to set the prediction
    ...
    embedding_algorithm = 'max_univariate_prediction_plus_lags'
    max_ccm_lag = 20
    ...
    sweep_delays = range(1, 20)
    
    n_ccm_bootstraps = 100
...
def main():
    ...
    with sqlite3.connect(SIM_DB_PATH) as db:
        for job_id, eps, beta00, sigma01, sd_proc, corr_proc, replicate_id, random_seed in db.execute(
            'SELECT * FROM job_info WHERE replicate_id < ?', [N_REPLICATES]
        ):
            job_dir = os.path.join(
                'jobs',
                '-'.join([
                    'corr_proc={}'.format(corr_proc)
                ]),
