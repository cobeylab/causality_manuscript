...
def main():
    ...
    assert ccm_settings['embedding_algorithm'] == 'max_univariate_prediction_plus_lags'
...
def run_analysis(cname, cause, ename, effect, db, rng, ccm_settings):
    ...
    if cause_sd == 0.0 or effect_sd == 0.0:
        ...
        run_analysis_max_univariate_prediction_plus_lags(cname, cause, ename, effect, theiler_window, db, rng, ccm_settings)

def run_analysis_max_univariate_prediction_plus_lags(cname, cause, ename, effect, theiler_window, db, rng, ccm_settings):
    ...
    for lag in range(-max_ccm_lag, max_ccm_lag + 1):
        ...
        corrs, pvalue_increase, pvalue_positive = run_analysis_for_embedding(
            cname, cause, ename, effect, max_corr_emb, theiler_window, ccm_settings['n_ccm_bootstraps'], db, rng
        )
